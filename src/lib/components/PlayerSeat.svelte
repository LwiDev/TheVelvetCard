...
<div
	class="flex flex-col items-center gap-1 lg:gap-1.5 min-w-[90px] max-w-[110px] px-2 py-1.5 lg:min-w-[140px] lg:max-w-[160px] lg:px-5 lg:py-3 rounded-xl border transition-all duration-200 relative select-none shadow-[0_6px_28px_rgba(0,0,0,0.55)]"
...
>
	<!-- Badges -->
	<div class="flex gap-1 min-h-[14px] lg:min-h-[20px] flex-wrap justify-center">
		{#if isDealer}
			<span class="text-[8px] lg:text-[11px] font-extrabold rounded-full px-1 lg:px-2 leading-[14px] lg:leading-[20px] bg-[#e8e0d0] text-[#1a1208]">D</span>
		{/if}
		{#if isSB}
			<span class="text-[8px] lg:text-[11px] font-extrabold rounded-full px-1 lg:px-2 leading-[14px] lg:leading-[20px] bg-gold/15 text-gold-light border border-gold/40">SB</span>
		{/if}
		{#if isBB}
			<span class="text-[8px] lg:text-[11px] font-extrabold rounded-full px-1 lg:px-2 leading-[14px] lg:leading-[20px] bg-gold/25 text-gold-light border border-gold/50">BB</span>
		{/if}
		{#if allIn}
			<span class="text-[8px] lg:text-[11px] font-extrabold rounded-full px-1 lg:px-2 leading-[14px] lg:leading-[20px] bg-[rgba(178,34,34,0.25)] text-[#f87171] border border-[rgba(178,34,34,0.45)]">ALL·IN</span>
		{/if}
	</div>

	<!-- Name -->
	<div class="text-[11px] lg:text-[15px] font-semibold max-w-[80px] lg:max-w-[130px] truncate font-body"
	...
	<!-- Stack -->
	<div class="text-[11px] lg:text-[15px] font-bold font-mono tracking-tight text-gold-light">
		{player.chips.toLocaleString()} <span class="text-[11px] opacity-70">♦</span>
	</div>

	<!-- Bet -->
	{#if player.currentBet > 0}
		<div class="text-[10px] lg:text-[12px] font-mono text-muted">bet {player.currentBet}</div>
	{/if}

	<!-- Cards -->
	<div class="flex gap-1 mt-1 lg:gap-1.5 lg:mt-1.5">
		{#if player.holeCards === null}
			<PokerCard card={null} size="sm" />
			<PokerCard card={null} size="sm" />
		{:else if player.holeCards[0] === '??'}
			<PokerCard card="??" size="sm" />
			<PokerCard card="??" size="sm" />
		{:else}
